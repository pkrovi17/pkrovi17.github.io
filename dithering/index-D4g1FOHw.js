...
void main() {
  ...
  if (e == 1.0) {
    ...
    if (paramE > 0.5) {
      float modWave = sin((p.x + p.y) / max(0.001, paramF) + paramD + time);
      l = clamp(l + modWave * paramG, 0.0, 1.0);
    }
    ...
    if (modeB > 0.5) {
      float off = paramH / max(resolution.x, 1.0);
      d.r = texture(inputTexture, vec2(clamp(uv.x + off, 0.0, 1.0), uv.y)).r;
      d.b = texture(inputTexture, vec2(clamp(uv.x - off, 0.0, 1.0), uv.y)).b;
    }
    ...
    if (modeB > 0.5 && modeB < 1.5) gv.x *= 0.75;
    float radius = (e == 3.0 ? paramA : paramA * 0.42) * (modeA > 1.5 ? sqrt(l) : 1.0 - l * 0.45);
    float dist = length(gv);
    if (modeB > 1.5 && modeB < 2.5) dist = max(abs(gv.x), abs(gv.y));
    if (modeB > 2.5) dist = abs(gv.y);
    float dotMask = smoothstep(radius, radius - 0.08, dist);
    if (paramE > 0.5) dotMask = 1.0 - dotMask;
    color = customMix(color * dotMask, dotMask);
  } else if (e == 4.0) {
    vec2 axis = paramE < 0.5 ? vec2(1.0, 0.0) : paramE < 1.5 ? vec2(0.0, 1.0) : normalize(vec2(1.0, 1.0));
    float row = floor(dot(uv * resolution, vec2(axis.y, axis.x)) / max(1.0, paramA));
    float gate = step(paramB, modeB > 0.5 ? 1.0 - l : l);
    float rnd = hash(vec2(row, floor(time * 8.0))) - 0.5;
    float dir = modeA > 0.5 ? -1.0 : 1.0;
    float shift = dir * gate * paramC * (0.35 + rnd * paramD);
    vec2 suv = uv + axis * vec2(shift / max(resolution.x, 1.0), shift / max(resolution.y, 1.0));
    color = texture(inputTexture, clamp(suv, vec2(0.0), vec2(1.0))).rgb;
  } else if (e == 6.0) {
    ...
    float edgeMask = modeA > 0.5 ? 1.0 - mask : mask;
    color = paramC > 0.5 ? mix(colorB, colorA, edgeMask) : vec3(edgeMask);
  } else if (e == 8.0) {
    ...
    vec2 cellUv = fract(uv * blocks);
    vec2 buv = floor(uv * blocks) / blocks + 0.5 / blocks;
    vec3 blockColor = texture(inputTexture, clamp(buv, vec2(0.0), vec2(1.0))).rgb;
    color = blockColor;
    ...
    if (modeB > 0.5 && modeB < 1.5) color = border > 0.0 ? colorA : texture(inputTexture, uv).rgb;
    else if (modeB > 1.5) color *= 0.75 + 0.25 * step(0.5, hash(floor(uv * blocks)));
    ...
    if (paramE > 0.5) q = 1.0 - q;
    vec3 thresholdColor = (paramC > 0.5 && paramC < 1.5) ? mix(colorB, colorA, q) : vec3(q);
    color = paramC > 1.5 ? mix(color, thresholdColor, 0.85) : thresholdColor;
  } else if (e == 11.0) {
    ...
    for (int octave = 0; octave < 8; octave++) {
      ...
      vec2 np = uv * scale * pow(2.0, float(octave)) + floor(time * paramD);
      float sampleNoise = modeB < 0.5 ? hash(floor(np)) : modeB < 1.5 ? hash(np) : modeB < 2.5 ? 1.0 - length(hash2(floor(np)) - fract(np)) : 0.5 + 0.5 * sin(hash(floor(np)) * 6.28318 + np.x + np.y);
      n += sampleNoise * amp;
      amp *= 0.5;
    }
    ...
    float speed = paramC;
    float lane = modeB < 1.5 ? grid.x : grid.y;
    float maxTravel = modeB < 1.5 ? resolution.y / cell : resolution.x / cell;
    float seed = hash(vec2(lane, floor(time * speed * 0.35)));
    float head = mod(seed * maxTravel + time * speed * 12.0, maxTravel);
    float pos = modeB < 0.5 ? grid.y : modeB < 1.5 ? maxTravel - grid.y : modeB < 2.5 ? grid.x : maxTravel - grid.x;
    float rain = smoothstep(trail, 0.0, abs(pos - head));
    rain *= step(1.0 - clamp(paramG, 0.0, 1.0), hash(vec2(lane, floor(time * speed))));
    float glyphSeed = hash(grid + floor(time * speed * 4.0) + paramH);
    float strokeA = smoothstep(0.08, 0.12, abs(local.x - 0.5));
    float strokeB = smoothstep(0.08, 0.12, abs(local.y - 0.5));
    float slash = smoothstep(0.06, 0.1, abs(local.x - local.y));
    float glyph = glyphSeed < 0.33 ? 1.0 - strokeA : glyphSeed < 0.66 ? 1.0 - strokeB : 1.0 - slash;
    glyph *= step(paramB / max(cell, 1.0), local.x) * step(paramB / max(cell, 1.0), local.y);
    color = mix(color * (1.0 - paramE), colorA * (0.35 + paramF * rain), rain * glyph);
  } else if (e == 13.0) {
